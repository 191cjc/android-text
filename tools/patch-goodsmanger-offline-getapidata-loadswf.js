...
const backupPath = path.join(projectRoot, "modified", "L4399Main_gamefile.before-goodsmanger-offline-getapidata-patch.swf");
...
function buildOfflineGetApiDataCode(abc, targetLength) {
  const gm = findQNameIndex(
    abc,
    (value) => value.endsWith("::GM"),
    "GM"
  );
  const loadSwf = findQNameIndex(
    abc,
    (value) => value.endsWith("::loadSwf"),
    "loadSwf"
  );
  ...
  const code = Buffer.concat([
    ...
    insn(0x4f, loadSwf, 0), // callpropvoid loadSwf, 0
    Buffer.from([0x47]), // returnvoid
  ]);

  if (code.length > targetLength) {
    throw new Error(`Offline GoodsManger.getApiData code ${code.length} exceeds target length ${targetLength}`);
  }
...
function patch() {
  ...
  for (const tag of findDoAbcTags(swf.body)) {
    ...
    const body = methodBodyFor(abc, "hotpointgame.Control::GoodsManger::static::::getApiData");
    if (!body) {
      continue;
    }

    const before = disassembleWindow(abc, body, 0, body.code.length);
    const newCode = buildOfflineGetApiDataCode(abc, body.code.length);
    ...
    return {
      ...
      inputPath,
      backupPath,
      method: "hotpointgame.Control::GoodsManger::static::::getApiData",
  ...
  throw new Error("GoodsManger.getApiData body was not found");
}
