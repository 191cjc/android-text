...
const backupPath = path.join(projectRoot, "modified", "L4399Main_gamefile.before-ggetallcong-offline-enter-gameb-patch.swf");
...
function buildOfflineOpenCode(abc, targetLength) {
  const gm = findQNameIndex(
    abc,
    (value) => value.endsWith("::GM"),
    "GM"
  );
  const enterGameB = findQNameIndex(
    abc,
    (value) => value.endsWith("::enterGameB"),
    "enterGameB"
  );
  ...
  const code = Buffer.concat([
    ...
    insn(0x4f, enterGameB, 0), // callpropvoid enterGameB, 0
    Buffer.from([0x47]), // returnvoid
  ]);

  if (code.length > targetLength) {
    throw new Error(`Offline GGetAllCong.open code ${code.length} exceeds target length ${targetLength}`);
  }
...
function patch() {
  ...
  for (const tag of findDoAbcTags(swf.body)) {
    ...
    const body = methodBodyFor(abc, "hotpointgame.gview::GGetAllCong::static::::open");
    if (!body) {
      continue;
    }

    const before = disassembleWindow(abc, body, 0, body.code.length);
    const newCode = buildOfflineOpenCode(abc, body.code.length);
    ...
    return {
      ...
      inputPath,
      backupPath,
      method: "hotpointgame.gview::GGetAllCong::static::::open",
  ...
  throw new Error("GGetAllCong.open body was not found");
}
