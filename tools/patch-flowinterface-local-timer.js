...
const backupPath = path.join(projectRoot, "modified", "L4399Main_gamefile.before-local-timer-patch.swf");
...
function buildLocalTimerCode(abc, targetLength) {
  const getTimer = findQNameIndex(
    abc,
    (value) => value.endsWith("::getTimer"),
    "flash.utils::getTimer"
  );

  const code = Buffer.concat([
    Buffer.from([0xd0, 0x30]), // getlocal0, pushscope
    insn(0x5d, getTimer), // findpropstrict getTimer
    insn(0x46, getTimer, 0), // callproperty getTimer, 0
    Buffer.from([0x48]), // returnvalue
  ]);

  if (code.length > targetLength) {
    throw new Error(`Local timer code ${code.length} exceeds target length ${targetLength}`);
  }
...
function patch() {
  ...
  for (const tag of findDoAbcTags(swf.body)) {
    ...
    const body = methodBodyFor(abc, "hotpointgame.Control::FlowInterface::static::::getCurrTimer");
    if (!body) {
      continue;
    }

    const before = disassembleWindow(abc, body, 0, body.code.length);
    const newCode = buildLocalTimerCode(abc, body.code.length);
    ...
    return {
      ...
      inputPath,
      backupPath,
      method: "hotpointgame.Control::FlowInterface::static::::getCurrTimer",
  ...
  throw new Error("FlowInterface.getCurrTimer body was not found");
}
